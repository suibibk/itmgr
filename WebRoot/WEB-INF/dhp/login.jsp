	...
	$.ajax({ 
		...
		success: function (data) {
			if(data.state=="success"){
				window.location.href='${pageContext.request.contextPath}/dhp/dhpAction!dhp.action';
			    return;
			}else{
				$("#tip").html("用户名或密码错误");
				one=true;
...
function  toRegister(){
	$("#login").hide();
	$("#register").show();
	$("#tip2").html("审核请联系林蛋黄QQ:1196216842");
}
...
function register(){
	...
	$.ajax({ 
		url: "${pageContext.request.contextPath }/dhp/dhpAction!register.action",
		type: "POST", 
